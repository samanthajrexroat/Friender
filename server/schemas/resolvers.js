...
const resolvers = {
  Query: {
    ...
    hobbyFans: async (parent, { hobbyId }) => {
      if (hobbyId) {
        return await User.find({ hobbies: hobbyId })
          .populate("hobbies")
          .populate("friends");
      }
    },
  ...
  Mutation: {
    ...
    createUser: async (
      parent,
      { firstName, lastName, email, password, city, age, description, photo }
    ) => {
    ...
    updateUser: async (
      parent,
      { userId, firstName, lastName, email, city, age, description },
      context
    ) => {
      try {
        if (userId) {
